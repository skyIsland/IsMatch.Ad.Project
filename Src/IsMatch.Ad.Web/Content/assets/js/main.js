...
function getPoint() {
	...
	} else if ($("#inputname").html() == '歌曲ID' || $("#inputname").html() == '歌曲ＩＤ' || $("#inputname").html() == '视频ID' || $("#inputname").html() == '视频ＩＤ' || $("#inputname").html() == '作品ID' || $("#inputname").html() == '作品ＩＤ' || $("#inputname").html() == '帖子ID' || $("#inputname").html() == '用户ID' || $("#inputname").html() == '用户ＩＤ') {
	...
	} else if ($("#inputname").html() == '作品链接' || $("#inputname").html() == '视频链接' || $("#inputname").html() == '分享链接') {
	...
	$.ajax({
		...
		success: function (data) {
			...
			if (data.code == 0) {
				...
				if (data.complain) {
					item += '<tr><td class="info" class="orderTitle">订单操作</td><td class="orderContent"><a href="./user/workorder.php?my=add&orderid=' + id + '&skey=' + skey + '" target="_blank" onclick="return checklogin(' + data.islogin + ')" class="btn btn-xs btn-default">投诉订单</a></td></tr>';
				}
...
function dopay(type, orderid) {
	if (type == 'rmb') {
			...
			success: function (data) {
				...
				if (data.code == 1) {
					...
					var confirmobj = layer.confirm('你的余额不足，请充值！', {
						...
						parent.location.href = './user/#chongzhi';
					}, function () {
					...
					var confirmobj = layer.confirm('你还未登录，是否现在登录？', {
						...
						parent.location.href = './user/login.php';
					}, function () {
						parent.location.href = './user/reg.php';
					}, function () {
						layer.close(confirmobj);
					});
				} else {
		...
		parent.location.href = 'other/submit.php?type=' + type + '&orderid=' + orderid;
	}
}
...
function checkInput() {
	if ($("#inputname").html() == '歌曲ID' || $("#inputname").html() == '歌曲ＩＤ' || $("#inputname").html() == '视频ID' || $("#inputname").html() == '视频ＩＤ' || $("#inputname").html() == '作品ID' || $("#inputname").html() == '作品ＩＤ' || $("#inputname").html() == '帖子ID' || $("#inputname").html() == '用户ID' || $("#inputname").html() == '用户ＩＤ') {
	...
	else if ($("#inputname").html() == '视频链接' || $("#inputname").html() == '作品链接' || $("#inputname").html() == '分享链接') {
		if ($("#inputvalue").val() != '' && $("#inputvalue").val().indexOf('http') >= 0) {
			getsharelink();
		}
	}
