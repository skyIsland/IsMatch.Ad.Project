...
namespace XCode.Membership
{
    ...
    public partial class Parameter : Entity<Parameter>
    {
        ...
        public override void Valid(Boolean isNew)
        {
            ...
            // 检查唯一索引
            // CheckExist(isNew, nameof(UserID), nameof(Category), nameof(Name));
        }
        ...
        {
            // 实体缓存
            if (Meta.Session.Count < 1000) return Meta.Cache.Find(e => e.UserID == userId && e.Category == category && e.Name == name);
        ...
        {
            // 实体缓存
            if (Meta.Session.Count < 1000) return Meta.Cache.FindAll(e => e.Category == category && e.Name == name);
        ...
        public static IList<Parameter> Search(Int32 userId, String category, String name, DateTime start, DateTime end, String key, PageParameter page)
        {
            ...
            if (!key.IsNullOrEmpty()) exp &= _.Value.Contains(key) | _.LongValue.Contains(key) | _.Ex4.Contains(key) | _.Ex5.Contains(key) | _.Ex6.Contains(key) | _.CreateUser.Contains(key) | _.CreateIP.Contains(key) | _.UpdateUser.Contains(key) | _.UpdateIP.Contains(key) | _.Remark.Contains(key);
